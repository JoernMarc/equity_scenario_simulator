 ...
 */
import type { Transaction, CapTable, CapTableEntry, Shareholding, ConvertibleLoanTransaction, FinancingRoundTransaction, ShareClass, WaterfallResult, WaterfallDistribution, VestingSchedule, VotingResult, VoteDistributionEntry, ShareTransferTransaction, Language, DebtInstrumentTransaction, UpdateShareClassTransaction, EqualizationPurchaseTransaction } from '../types';
...
export function calculateCapTable(transactions: Transaction[], asOfDate: string, excludeTransactionId?: string): CapTable {
  ...
  for (const tx of relevantTransactions) {
    switch (tx.type) {
      ...
      case 'EQUALIZATION_PURCHASE': {
        const eqTx = tx as EqualizationPurchaseTransaction;
        
        const newHolding: Shareholding = {
          id: crypto.randomUUID(),
          stakeholderId: eqTx.newStakeholderId,
          stakeholderName: eqTx.newStakeholderName,
          shareClassId: eqTx.shareClassId,
          shares: eqTx.purchasedShares,
          investment: eqTx.purchasedShares * eqTx.pricePerShare,
          originalPricePerShare: eqTx.pricePerShare,
        };
        
        currentShareholdings.push(newHolding);
        break;
      }
    }
  }
